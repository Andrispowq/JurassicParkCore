...
public class RouteValidator(IGameService gameService) : IRouteValidator
{
    public async Task<bool> IsComplete(JeepRoute route, Position start, Position end)
    {
        await gameService.LoadCollection(route, r => r.RoutePositions);
        ...
        return await Validate(route) == RouteValidityReason.Valid;
    }

    public async Task<RouteValidityReason> Validate(JeepRoute route)
    {
        await gameService.LoadCollection(route, r => r.RoutePositions);
        var positions = route.RoutePositions;
    ...
    public async Task<bool> CanAddRouteBlock(JeepRoute route, Position position)
    {
        await gameService.LoadCollection(route, r => r.RoutePositions);
        var positions = route.RoutePositions;
    ...
    public async Task<bool> CanRemoveElement(JeepRoute route)
    {
        await gameService.LoadCollection(route, r => r.RoutePositions);
        return route.RoutePositions.Count > 0;
    }

    public async Task<bool> IsPositionOnRoute(JeepRoute route, Position position)
    {
        await gameService.LoadCollection(route, r => r.RoutePositions);
    ...
    public async Task<bool> IsPositionTaken(Position position)
    {
        if (!position.IsRoundedDown) return false;
        
        var positions = await gameService.GetPositions();
        return positions
            .Where(p => p.JeepRouteId != null)
    ...
    private static RouteValidityReason IsValidRoute(ICollection<Position> positions)
    {
        ...
        for (int i = 0; i < n - 1; i++)
        {
            for (int j = i + 1; j < n - 1; j++)
            {
                ...
                if (DoIntersect(pts[i], pts[i + 1], pts[j], pts[j + 1])) return RouteValidityReason.SelfIntersecting;
            }
        }
