    ...
    IRandomValueProvider randomValueProvider) : IGameService
{
    ...
    public async Task<Option<ServiceError>> SellAnimal(SavedGame game, Animal animal, decimal refundPrice)
    {
        if (game.GameState != GameState.Ongoing)
        {
            return new UnauthorizedError("Game is over");
        ...
        try
        {
            var animalResult = await animalService.DeleteAnimal(db, animal);
            if (animalResult is Option<ServiceError>.Some animalError)
            {
                await transaction.RollbackAsync();
                return new ConflictError(animalError.Value.Message);
    ...
    public async Task<Option<ServiceError>> UpdateGroup(AnimalGroup group)
    {
        await using var db = await dbContextFactory.CreateDbContextAsync();
        return await animalService.UpdateGroup(db, group);
    }
    ...
    public async Task<Option<ServiceError>> SellJeep(SavedGame game, Jeep jeep, decimal refundPrice)
    {
        if (game.GameState != GameState.Ongoing)
        {
            return new UnauthorizedError("Game is over");
        ...
        try
        {
            var jeepResult = await jeepService.DeleteJeep(db, jeep);
            if (jeepResult is Option<ServiceError>.Some jeepError)
            {
                await transaction.RollbackAsync();
                return new ConflictError(jeepError.Value.Message);
    ...
    public async Task<Option<ServiceError>> SellMapObject(SavedGame game, MapObject mapObject, decimal refundPrice)
    {
        if (game.GameState != GameState.Ongoing)
        {
            return new UnauthorizedError("Game is over");
        ...
        try
        {
            var objectResult = await mapObjectService.DeleteMapObject(db, mapObject);
            if (objectResult is Option<ServiceError>.Some objError)
            {
                await transaction.RollbackAsync();
                return new ConflictError(objError.Value.Message);
